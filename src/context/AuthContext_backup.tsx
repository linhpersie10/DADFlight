...
const PROFILE_FIELDS_TO_MERGE = [
  'employeeCode', 'employeeId', 'fullName', 'displayName', 'photoURL',
  'role', 'status', 'isAdmin', 'isSuperadmin', 'isApproved', 'isRejected',
  'departmentId', 'title', 'level', 'pinCode', 'hasPin', 'pinConfigured',
  'requestedApps', 'microsoftEmail', 'createdAt', 'updatedAt'
];

async function absorbOtherProfiles(
  currentUid: string,
  email: string,
  currentData: Record<string, any>
): Promise<Record<string, any>> {
  if (!email || !db) return currentData;
  const normalizedEmail = email.toLowerCase().trim();

  const snap = await getDocs(
    query(collection(db, 'PKT_DAD_users'), where('email', '==', normalizedEmail))
  );
  const otherDocs = snap.docs.filter(d => d.id !== currentUid);
  if (otherDocs.length === 0) return currentData;

  const currentRef = doc(db, 'PKT_DAD_users', currentUid);
  let merged = { ...currentData };

  for (const other of otherDocs) {
    const otherUid = other.id;
    const otherData = other.data() as Record<string, any>;
    const updateFields: Record<string, any> = {};

    for (const field of PROFILE_FIELDS_TO_MERGE) {
      const cur = merged[field];
      const oth = otherData[field];
      const curEmpty = cur === undefined || cur === null || cur === '';
      const othFilled = oth !== undefined && oth !== null && oth !== '';
      if (curEmpty && othFilled) {
        updateFields[field] = oth;
        merged[field] = oth;
      }
    }
    
    // Status logic: approved wins
    if (otherData.status === 'approved' && merged.status !== 'approved') {
    ...
    // Role logic: admin wins
    if (otherData.role === 'admin' && merged.role !== 'admin') {
    ...
    // requestedApps merge
    if (Array.isArray(otherData.requestedApps)) {
      const curApps = Array.isArray(merged.requestedApps) ? merged.requestedApps : [];
      const newApps = [...new Set([...curApps, ...otherData.requestedApps])];
      if (newApps.length > curApps.length) {
        updateFields.requestedApps = newApps;
        merged.requestedApps = newApps;
      }
    }

    if (Object.keys(updateFields).length > 0) {
      await setDoc(currentRef, updateFields, { merge: true }).catch(() => {});
    }

    // Migrate PIN secret if exists
    try {
      const oldPinRef = doc(db, 'PKT_DAD_users', otherUid, 'private', 'pin');
      const oldPinSnap = await getDoc(oldPinRef);
      if (oldPinSnap.exists()) {
        const newPinRef = doc(db, 'PKT_DAD_users', currentUid, 'private', 'pin');
        await setDoc(newPinRef, oldPinSnap.data(), { merge: true });
        await deleteDoc(oldPinRef);
      }
    } catch (e) {
      console.warn(`[Auth] Failed to migrate PIN for ${otherUid}:`, e);
    }

    await deleteDoc(doc(db, 'PKT_DAD_users', otherUid)).catch(() => {});
    console.info(`[Auth] absorbOtherProfiles: "${otherUid}" → "${currentUid}"`);
  }
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        try {
          ...
          // NEW: Background profile absorption
          try {
            const uSnap = await getDoc(userRef);
            await absorbOtherProfiles(
              currentUser.uid,
              currentUser.email || '',
              uSnap.exists() ? uSnap.data() : {}
            );
          } catch (e) {
            console.warn('[Auth] Background profile absorption failed:', e);
          }
